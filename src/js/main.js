$(document).ready(function(){
			...
			case 'Chat':
				...
				$('#chatBox input').bind('keydown', function(e) {
					...
					msg.charAt(0) == '/' ?
						...
						jsonApi.call('broadcastWithName', [msg, user + '_opcraft'], clearInput);
				});
	...
	function init() {
		...
		//alert('http://minecraft.paulstraw.com:20059/api/subscribe?source=all&key=' + generateKey('all'));
			...
			message: function(e) {
				...
				switch (data.source) {
					...
					case 'chat':
						...
						break;
					case 'console':
						if (~result.line.search(/<\w+>/g) || ~result.line.indexOf('method=broadcastWithName')) return; //if this is a chat log, don't worry about it
	...
	(function() {
		if (typeof localStorage == 'undefined') {
			...
			document.write('Your browser doesn\'t support WebSockets (which are required by Crafty). You should upgrade to something like <a href="http://google.com/chrome">Google Chrome</a>. You can also try installing Flash if you don\'t want to get a new browser.');
			return;
		}
